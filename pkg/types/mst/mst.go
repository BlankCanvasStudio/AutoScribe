...
func Document(f FunctionDetails, prompt, ApiKey string) error {
	...
	var DocumentedComment string

	for {
		// DocumentationString, err := call.Query41Nano(fullPrompt, ApiKey, nil)
		DocumentationString, err := call.Query5Nano(fullPrompt, ApiKey, nil)
		if err != nil {
		...
		// If it doesn't return a valid go comment, try again
		// There are so many ways this goes wrong and given how */ operates, we can't just
		//   blanket wrap the response
		DocumentedComment, err = formatting.FormatAsGoComment(DocumentationString)
		if err != nil {
			log.Errorf("failed to parse for comments: %v", err)
			// Not a valid go comment. Try again
			continue
		}

		// Got a valid go comment
		break
	}
